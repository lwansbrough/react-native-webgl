...
- (id)initWithFrame:(CGRect)frame {
  if (self = [super init]) {
    ...
    // TODO: Should maybe use a share group for the context creation
    context = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES2];
    
    bufferHeight = height = frame.size.height;
    bufferWidth = width = frame.size.width;
...
- (BOOL)needsPresenting {
  return self.needsPresenting;
}

- (void)setNeedsPresenting:(BOOL)needsPresenting {
